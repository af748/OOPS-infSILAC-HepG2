...
```{r}
#need to 2^ value in order to un-log the value
#need to -0.5 the ratio.
#in this chunk i attempt to change the value column from log values back to the ratio, and i attempt to -0.5 from the value.
  
  
#Here i attempt to raise the ratio to the power of 10 (opposite of log)
#should be opposite of log2 which is 2^value
silac_unlog <- silac_protein_tidy %>%
  mutate(silac_protein_tidy, value= 2^value)

#Here i attempt to subtract 0.5
silac_0.5 <- 
   mutate(silac_unlog, value=value-0.5)
#this just made things a bit easier 
silac_protein_tidy <- silac_0.5
...
```{r}
prot_count<- rowSums(is.finite(exprs(prot_res_flt)))
