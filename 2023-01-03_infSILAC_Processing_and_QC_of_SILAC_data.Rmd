...
ratios_matched <- ratios %>%
  ...
  # (rep1-8, CL=L; rep9-16 CL=H)
  mutate('Matched_CL' = ifelse(Replicate %in% 9:16, matched_Heavy, matched_Light),
         'Matched_NC' = ifelse(Replicate %in% 9:16, matched_Light, matched_Heavy)) %>%
...
cl_nc_correlations %>%
  ggplot(aes(matched, cor, colour=ifelse(Replicate %in% 1:8, '1:8', '9:16'))) +
  geom_point() +
...
```{r}
plot_quant(pep_res_flt[,order(pData(pep_res_flt)$Replicate)])
plot_quant(prot_res[,order(pData(prot_res)$Replicate)])
```
